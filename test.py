...
def processing_message(update: Update, context: CallbackContext) -> int:
    return context.bot.send_message(chat_id=update.message.chat_id, text="Processing...").message_id

def delete_processing_message(update: Update, context: CallbackContext, message_id: int) -> None:
    context.bot.delete_message(chat_id=update.message.chat_id, message_id=message_id)

def ocr_translate(img, target_language, update: Update, context: CallbackContext):
    processing_msg_id = processing_message(update, context)
    text = pytesseract.image_to_string(img)
    if target_language not in googletrans.LANGUAGES:
        delete_processing_message(update, context, processing_msg_id)
        return f"Error: Invalid target language '{target_language}'. Please choose a valid language code."
    try:
        lang = translator.detect(text).lang
        translation = translator.translate(text, dest=target_language).text
...
def handle_image(update: Update, context: CallbackContext) -> None:
    ...
    target_language = context.user_data.get("target_language", "en")
    reply_text = ocr_translate(img, target_language, update, context)
    delete_processing_message(update, context, processing_msg_id)
...
def handle_document(update: Update, context: CallbackContext) -> None:
    ...
    if document.file_name.lower().endswith('.pdf'):
        ...
        for idx, img in enumerate(images):
            target_language = context.user_data.get("target_language", "en")
            translation_text = ocr_translate(img, target_language, update, context)
